	...
	, double max = std::numeric_limits<double>::max())
{
	...
	for (int j = 0; j<NTRY; j++) {
		...
		if (std::abs(f1) < std::abs(f2))
			f1 = f(a = fmax(a + FACTOR*(a - b), min));
		else
			f2 = f(b = fmin(b + FACTOR*(b - a), max));
		std::cout << "      current  (a, b) = (" << a << "," << b << ") " << "; (fa, fb) = (" << f1 << "," << f2 << ") " << std::endl;
	}
	return false;
}
...
{
	std::cout << "   Start brent root solver ..." << std::endl;
	const int ITMAX = 100;
