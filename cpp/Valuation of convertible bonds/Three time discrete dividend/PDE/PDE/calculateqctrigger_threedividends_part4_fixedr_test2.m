function difference = calculateqctrigger_threedividends_part4_fixedr_test2(S0, Shistory, ctrigger, qctrigger,T,r,sigma,no_call_time,n,m,ncX,D1_time,D1,D2_time,D2,D3_time,D3)
%% parameter input
% clc;
% clear;
% S0=18.21;
...
for i=1:2*N
    ...
    Stmp3=S(D3_time/d_t-1).*exp((r-sigma^2/2)*d_t+sigma*sqrt(d_t)*sam(i,D3_time/d_t));
    S(D3_time/d_t)=Stmp3-D3;
    for j=D3_time/d_t+1:M
        S(j)=S(j-1).*exp((r-sigma^2/2)*d_t+sigma*sqrt(d_t)*sam(i,j));
    end
    %S=10*ones(1,M);
    SS=[Shistory,S0,S];
    ...
    ctrigger_sequence(D2_time/d_t:D3_time/d_t-1+m)=ctrigger-ncX*(D1+D2);
    ctrigger_sequence(D3_time/d_t:M+m)=ctrigger-ncX*(D1+D2+D3);
    ...
    if T+(m-1)*d_t<=no_call_time(1)-d_t
        ...
        if left_j>m
            ...
            if left_triggertime>=D3_time  %·Öºìºó
