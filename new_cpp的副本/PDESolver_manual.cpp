...
double PDESolver::getA(double arcs[N][N],int n)
{
    if(n==1)
    {
        return arcs[0][0];
...
void  PDESolver::getAStart(double arcs[N][N],int n,double ans[N][N])
{
    if(n==1)
    {
        ans[0][0] = 1;
...
bool PDESolver::GetMatrixInverse(double src[N][N],int n,double des[N][N])
{
    ...
    {
        this->getAStart(src,n,t);
        for(int i=0;i<n;i++)
        {
            for(int j=0;j<n;j++)
            {
                des[i][j]=t[i][j]/flag;
                // res[i][j] = des[i][j]; //用于存储已经计算好的逆矩阵
                cout<<des[i][j]<<"\t";
            }
            cout<<endl;
...
vector<vector<double>*> PDESolver::MatMul(double left[N][M],double right[M][N])
{

    vector<vector<double>*> res; //有N个 row_res 横向叠放 形状为 N x N
    
    vector<double> row_res; //每一个元素都是 左矩阵一行与右矩阵一列相乘再求和的结果 形状为 N x 1
    int i = 0;
    while(i<N)
    {
        double sum=0;
        ...
        while(j < M)
        {
            sum += left[i][j]*right[j][i];
            j++;
        }
        row_res.push_back(sum);
        cout<<sum<<" ";
        i ++;
    }
    return res;
...
double PDESolver::solve()
{
    ...
    for(int n=1;n<2;n++)
    {
        ...
        cout<<"u[225]= "<<this->pde_param_ptr->u[225]<<endl;
        cout<<"u[2390]= "<<this->pde_param_ptr->u[2390]<<endl;
    }
